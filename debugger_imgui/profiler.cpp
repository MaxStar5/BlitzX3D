...
Profiler::Profiler() :freq(0), startTicks(0), lastMemSampleMs(0), lastWorkingSetBytes(0), enabled(true),
	lastSampleTick(0), sampleIntervalMs(1.0) {
	LARGE_INTEGER f;
...
void Profiler::sampleStack() {
	...
	if (sampleIntervalMs > 0.0 && lastSampleTick) {
		if (ticksToMs(nowTicks() - lastSampleTick) < sampleIntervalMs) return;
	}
	lastSampleTick = nowTicks();
